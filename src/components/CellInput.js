...
var offsetFromViewport = function (elem) {
  ...
  if (is_in_viewport) {
    ...
    return {
      top:
        bounding.top < 0 ? -bounding.top : window.innerHeight - bounding.bottom,
    };
  }
};
...
export const CellInput = ({
          ...
          for (const package_match of inner.matchAll(inner_re)) {
            ...
            if (package_name !== "Base" && package_name !== "Core") {
              ...
              cm.setBookmark(
                {
                  line: line_i,
                  ch: start + package_match.index + package_match[0].length,
                },
                {
                  widget: widget,
                }
  ...
  useLayoutEffect(() => {
    const cm = (cm_ref.current = CodeMirror.fromTextArea(
      text_area_ref.current,
      {
        value: local_code,
        lineNumbers: true,
        mode: "julia",
    ...
    keys["Ctrl-/"] = () => {
      ...
      if (old_value === new_value) {
        ...
        cm.setValue(
          cm.lineCount() === 1 ? `# ${new_value}` : `#= ${new_value} =#`
        );
    ...
    const alt_move = (delta) => {
      ...
      const selected_lines = new Set(
        [].concat(
          ...selections.map((sel) => range(sel.anchor.line, sel.head.line))
        )
      );
    ...
    keys["Backspace"] = keys["Ctrl-Backspace"] = () => {
      ...
      if (cm.lineCount() === 1 && cm.getValue() === "") {
        ...
        let enough_time_passed_since_last_backspace =
          Date.now() - time_last_genuine_backspace.current >
          BACKSPACE_CELL_DELETE_COOLDOWN;
        let enough_time_passed_since_force_focus =
          Date.now() - time_last_being_force_focussed_ref.current >
          BACKSPACE_AFTER_FORCE_FOCUS_COOLDOWN;
        if (
          enough_time_passed_since_last_backspace &&
          enough_time_passed_since_force_focus
        ) {
    ...
    const at_first_line_visually = () =>
      isapprox(cm.cursorCoords(null, "div").top, 0.0);
    ...
    const at_first_position = () =>
      cm.findPosH(cm.getCursor(), -1, "char")?.hitSide === true;
    ...
    const at_last_line_visually = () =>
      isapprox(
        cm.cursorCoords(null, "div").top,
        cm.cursorCoords({ line: Infinity, ch: Infinity }, "div").top
      );
    ...
    const open_close_selection = (opening_char, closing_char) => () => {
      if (cm.somethingSelected()) {
        for (const selection of cm.getSelections()) {
          cm.replaceSelection(
            `${opening_char}${selection}${closing_char}`,
            "around"
          );
        }
      } else {
    ...
    ["()", "{}", "[]"].forEach((pair) => {
      ...
      keys[`'${opening_char}'`] = open_close_selection(
        opening_char,
        closing_char
      );
    });
      ...
      setTimeout(() => {
        ...
        if (cm.somethingSelected()) {
          ...
          if (
            ...
            let good_token = before_and_after_token.find((x) =>
              is_good_token(x)
            );
            if (good_token) {
              let tokens = cm.getLineTokens(cursor.line);
              let current_token = tokens.findIndex(
                (x) => x.start === good_token.start && x.end === good_token.end
              );
              on_update_doc_query(
    ...
    cm.on("mousedown", (cm, e) => {
      ...
      if (
        has_ctrl_or_cmd_pressed(e) &&
        which === 1 &&
        isVariable &&
        used_variables.includes(varName)
      ) {
        e.preventDefault();
  ...
  useEffect(() => {
    if (cm_forced_focus == null) {
      ...
      time_last_being_force_focussed_ref.current = Date.now();
      let cm_forced_focus_mapped = cm_forced_focus.map((x) =>
        x.line === Infinity ? { ...x, line: cm_ref.current.lastLine() } : x
      );
...
const InputContextMenu = ({
  ...
  const toggle_is_in_app = async (e) => {
    const new_val = !running_disabled;
    e.preventDefault();
    e.stopPropagation();
    await pluto_actions.update_notebook((notebook) => {
  ...
  >
    ...
    ${open
      ? html`<ul onMouseenter=${mouseenter} className="input_context_menu">
          <li onClick=${on_delete} title="Delete">
            <span className="delete_icon" />Delete cell
          </li>
          <li
          ...
          >
            ${running_disabled
              ? html`<span className="enable_cell_icon" />`
              : html`<span className="disable_cell_icon" />`}
            ${running_disabled ? html`<b>Enable cell</b>` : html`Disable cell`}
          </li>
          <li className="coming_soon" title="">
            <span className="bandage_icon" /><em>Coming soon…</em>
          </li>
        </ul>`
      : html``}
  </button>`;
...
const on_tab_key = (cm) => {
  ...
  if (cm.somethingSelected()) {
    ...
    if (
      cursor.ch > 0 &&
      no_autocomplete.indexOf(old_line[cursor.ch - 1]) == -1
    ) {
      cm.showHint();
    } else {
      cm.replaceSelection("\t");
    ...
    .then(({ message }) => {
      const completions = {
        list: message.results.map(([text, type_description, is_exported]) => ({
          text: text,
          className:
            (is_exported ? "" : "c_notexported ") +
            (type_description == null ? "" : "c_" + type_description),
          // render: (el) => el.appendChild(observablehq_for_myself.html`<div></div>`),
        })),
        from: CodeMirror.Pos(
          cursor.line,
          utf8index_to_ut16index(old_line, message.start)
        ),
        to: CodeMirror.Pos(
          cursor.line,
          utf8index_to_ut16index(old_line, message.stop)
        ),
      };
...
const module_expanded_selection = ({
  tokens_before_cursor,
  tokens_after_cursor,
}) => {
  ...
  let i_guess_current_token =
    tokens_before_cursor[tokens_before_cursor.length - 1];
  if (
    i_guess_current_token?.type === "builtin" &&
    i_guess_current_token.string.startsWith("::")
  ) {
    let typedef_tokens = [];
