...
export const CellInput = ({
    ...
    return html`
        <pluto-input ref=${dom_node_ref}>
            <${InputContextMenu} on_delete=${on_delete} cell_id=${cell_id} run_cell=${on_submit} running_disabled=${running_disabled} is_in_app=${is_in_app} />
            <textarea ref=${text_area_ref}></textarea>
        </pluto-input>
    `
}

const InputContextMenu = ({ on_delete, cell_id, run_cell, running_disabled, is_in_app }) => {
    ...
    return html`<button onMouseLeave=${mouseleave} onClick=${() => setOpen(!open)} onBlur=${() => setOpen(false)} className="delete_cell" title="Actions">
        <span className="icon"></span>
        ${open
            ? html`<ul onMouseEnter=${mouseenter} className="input_context_menu">
                  <li onClick=${on_delete} title="Delete"><span className="delete_icon" />Delete cell</li>
                  <li
                  ...
                  >
                      ${running_disabled ? html`<span className="enable_cell_icon" />` : html`<span className="disable_cell_icon" />`}
                      ${running_disabled ? html`<b>Enable cell</b>` : html`Disable cell`}
                  </li>
                  <li onClick=${toggle_is_in_app} title=${is_in_app ? "Remove cell from the app grid" : "Add cell to the app grid"}>
                      ${is_in_app ? html`<span className="enable_cell_icon" />` : html`<span className="disable_cell_icon" />`}
                      ${is_in_app ? html`<b>Take from app</b>` : html`Add to app`}
                  </li>
                  <li className="coming_soon" title=""><span className="bandage_icon" /><em>Coming soon…</em></li>
              </ul>`
            : html``}
    </button>`
}
