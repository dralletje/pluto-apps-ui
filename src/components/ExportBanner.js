...
export const ExportBanner = ({ onClose, notebookfile_url, notebookexport_url }) => {
    return html`
        <aside id="export">
            ...
            <a className="export_card" style="border-color: #00000021; opacity: .7;">
                <header>mybinder.org</header>
                <section>Publish an interactive notebook online.</section>
            </a>-->
                <button title="Close" className="toggle_export" onClick=${() => onClose()}>
                    <span></span>
                </button>
            </div>
        </aside>
