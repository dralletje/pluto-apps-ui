...
export const BinderButton = ({ binder_phase, start_binder, notebookfile }) => {
    ...
    return html`<div id="launch_binder">
        <span
            ...
            className="explain_binder"
            ><b>Edit</b> or <b>run</b> this notebook</span
        >
        ${popupOpen &&
        html`<div id="binder_help_text">
            <span onClick=${() => setPopupOpen(false)} className="close"></span>
            ${show_binder
                ? html`
            ...
            <ol style="padding: 0 2rem;">
                <li>
                    <div>
                        ${recommend_download
                            ? html`
                                  <div className="command">Download the notebook:</div>
                                  <div
                                      onClick=${(e) => {
                              ...
                              `
                            : html`
                                  <div className="command">Copy the notebook URL:</div>
                                  <div className="copy_div">
                                      <input onClick=${(e) => e.target.select()} value=${notebookfile_ref.current} readonly />
                                      <span
                                          className=${`copy_icon ${showCopyPopup ? "success_copy" : ""}`}
                                          onClick=${async () => {
                ...
                <li>
                    <div className="command">Run Pluto</div>
                    <p>
                        ${"(Also see: "}
                ...
                <li>
                    ${recommend_download
                        ? html`
                              <div className="command">Open the notebook file</div>
                          ...
                          `
                        : html`
                              <div className="command">Paste URL in the <em>Open</em> box</div>
