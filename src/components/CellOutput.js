import React, { html, Component, useRef, useLayoutEffect, useContext, useEffect, useMemo } from "../imports/Preact.js"
...
export class CellOutput extends Component {
    constructor() {
        ...
        this.resize_observer = new ResizeObserver((entries) => {
            const new_height = this.base.current.offsetHeight
    ...
    componentDidMount() {
        this.resize_observer.observe(this.base.current)
    }

    componentWillUnmount() {
        this.resize_observer.unobserve(this.base.current)
    }
    ...
    render() {
        return html`
            ...
            >
                <pluto-assignee>${this.props.rootassignee}</pluto-assignee>
...
export const OutputBody = ({ mime, body, cell_id, persist_js_state = false, last_run_timestamp }) => {
    switch (mime) {
        ...
        case "text/plain":
            if (body) {
                return html`<div>
                    <pre className="no-block"><code>${body}</code></pre>
                </div>`
            } else {
                return html`<div></div>`
            }
...
export let RawHTMLContainer = ({ body, persist_js_state = false, last_run_timestamp }) => {
    ...
    return html`<div className="raw-html-wrapper" ref=${container}></div>`
}
