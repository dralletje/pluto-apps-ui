...
export const PkgPopup = ({ notebook }) => {
    ...
    const open = (e) => {
        ...
        pos_ref.current = {
            ...
            left: `min(max(0px,100vw - 251px - 30px), ${el.getBoundingClientRect().left - document.body.getBoundingClientRect().left}px);`,
        }
    ...
    >
        ...
        <div className="pkg-buttons">
            <a
                className="pkg-update"
                target="_blank"
                title="Update packages"
                style=${{
                    ...(Boolean(showupdate) ? {} : { opacity: 0.4 }),
                    ...(recent_event?.is_disable_pkg ? { display: `none` } : {}),
                }}
                href="#"
                onClick=${(e) => {
                    if (busy) {
                        ...
                        if (window.confirm("Would you like to check for updates and install them? A backup of the notebook file will be created.")) {
            ...
            <a
                ...
                style=${Boolean(terminal_value) ? {} : { display: `none` }}
                href="#"
                onClick=${(e) => {
            ...
            /></a>
            <a className="help" target="_blank" title="Go to help page" href="https://github.com/fonsp/Pluto.jl/wiki/%F0%9F%8E%81-Package-management"
