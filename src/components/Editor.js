...
export class Editor extends Component {
    constructor() {
        ...
        this.launch_params = {
            ...
            slider_server_url:
                //@ts-ignore
                url_params.get("slider_server_url") ?? window.pluto_slider_server_url,
        }
        ...
        this.actions = {
            ...
            confirm_delete_multiple: async (verb, cell_ids) => {
                if (cell_ids.length <= 1 || window.confirm(`${verb} ${cell_ids.length} cells?`)) {
                    if (cell_ids.some((cell_id) => this.state.notebook.cell_results[cell_id].running || this.state.notebook.cell_results[cell_id].queued)) {
                        if (window.confirm("This cell is still running - would you like to interrupt the notebook?")) {
...
#######################**************************########################`,
        ...
        if (this.state.static_preview) {
            ...
            fetch(`https://stats.plutojl.org/count?p=/article-view#skip_sw`, {
                cache: "no-cache",
            }).catch(() => {})
        } else {
            this.connect("ws://localhost:1234")
        }
        ...
        setInterval(() => {
            if (!this.state.static_preview && document.visibilityState === "visible") {
                // view stats on https://stats.plutojl.org/
                fetch(
                    `https://stats.plutojl.org/count?p=/editing/${
                        // @ts-ignore
                        window?.version_info?.pluto ?? "unknown"
                    }#skip_sw`,
                    { cache: "no-cache" }
                ).catch(() => {})
            }
        ...
        this.submit_file_change = async (new_path, reset_cm_value) => {
            ...
            if (!this.state.notebook.in_temp_dir) {
                if (!window.confirm("Are you sure? Will move from\n\n" + old_path + "\n\nto\n\n" + new_path)) {
    ...
    UNSAFE_componentWillUpdate(new_props, new_state) {
