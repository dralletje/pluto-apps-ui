...
const prefix = ({ prefix, prefix_short }) =>
    html`<pluto-tree-prefix><span className="long">${prefix}</span><span className="short">${prefix_short}</span></pluto-tree-prefix>`
...
export const TableView = ({ mime, body, cell_id, persist_js_state }) => {
    ...
    return html`<table className="pluto-table" ref=${node_ref}>
        ${thead}${tbody}
    </table>`
}
